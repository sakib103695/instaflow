...
function ArrowDown() {
  return (
    <svg width="20" height="20" fill="none" viewBox="0 0 21.6 21.6" className="shrink-0">
      <path
  ...
  return (
    <div className="relative shrink-0 size-[72px]">
      <svg className="block size-full" fill="none" viewBox="0 0 72 72">
        <circle cx="36" cy="36" r="35" fill="white" fillOpacity="0.06" stroke="white" strokeOpacity="0.1" strokeWidth="0.7" />
        <g>
          <path d={svgPaths.p1df84f80} stroke="white" strokeWidth="1.78" />
          <path d={svgPaths.p25e17200} stroke="white" strokeLinecap="round" strokeWidth="1.78" />
          <path d={svgPaths.p1f89b300} stroke="white" strokeLinecap="round" strokeWidth="1.78" />
        </g>
      </svg>
    </div>
  );
...
function VoiceBar() {
  return (
    <svg className="size-[45px] shrink-0 opacity-70" fill="none" viewBox="0 0 45 45">
      <path d={svgPaths.p702a680} stroke="white" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.56" />
    </svg>
  );
}
...
function VoiceLevelBars({ isActive }: { isActive: boolean }) {
  ...
  return (
    <div className="flex items-center justify-center gap-0.5 h-12 shrink-0" aria-hidden>
      {[...Array(bars)].map((_, i) => (
        <div
          key={i}
...
function HeroCard({
  ...
  return (
    <div
      className="relative w-full max-w-[560px] min-w-0 rounded-[16px] sm:rounded-[20px] p-4 sm:p-6 flex flex-col gap-4 sm:gap-6 overflow-hidden border border-white/10 backdrop-blur-xl shadow-[inset_0_1px_40px_0_rgba(227,222,255,0.2),inset_0_0_56px_-36px_rgba(255,255,255,0.5)] bg-white/[0.06]"
    >
      ...
      <div className="flex flex-col gap-4 relative z-10">
        <p className="text-[#7e8faa] text-xs uppercase tracking-widest">Select Voice Agent Profile</p>
          ...
          <div className="flex-1 min-w-0 sm:min-w-[200px] relative">
            ...
            >
              {AVAILABLE_VOICES.map((v) => (
                <option key={v.id} value={v.id} className="bg-[#1a0a2e] text-white">
                  {v.label}
                </option>
              ))}
            </select>
  ...
  />
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-4 py-2 relative z-10">
        ...
        >
          <div className={`absolute inset-0 rounded-full blur-2xl transition-opacity ${isActive ? "bg-[#CD9EFF]/40" : "bg-[#CD9EFF]/30"}`} />
          <CentralMic />
        </button>
        {isActive ? <VoiceLevelBars isActive={true} /> : <VoiceBar />}
      </div>
{/* 
      ...
      {isActive && transcription.length > 0 && (
        <div className="transcription-scroll relative z-10 pt-4 border-t border-white/10 space-y-2 max-h-40 overflow-y-auto">
          {transcription.slice(-6).map((line, idx) => (
            <div
              key={idx}
              className={`p-3 rounded-xl text-left text-sm ${
                line.startsWith(selectedVoice.label)
                  ? "bg-[#8c21ff]/20 border border-[#8c21ff]/30"
                  : "bg-white/5 border border-white/10"
              }`}
            >
              <p className="text-white/90 leading-snug">{line}</p>
            </div>
          ))}
        </div>
      )}
